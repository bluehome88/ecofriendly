...
class WC_XR_Line_Item {
	...
	public function get_tax_type() {
		...
		if ( $this->get_tax_amount() <= 0 ) {
			...
			if( $this->settings->get_option( 'match_zero_vat_tax_rates' ) ) {
				...
				$tax_type = self::get_tax_type_for_label_and_rate( $xero_tax_rates, $tax_rate['label'], $rate_to_find, $logger );
				if ( ! empty( $tax_type) ) {
	...
	protected static function get_tax_type_for_label_and_rate( $tax_rates, $label_to_find, $rate_to_find, $logger ) {
		...
		foreach ( $tax_rates as $tax_rate ) {
			$logger->write( " - Matching rate (Name) ({$tax_rate['name']}) and rate ({$tax_rate[ 'effective_rate' ]})" );
			if ( strcasecmp( $tax_rate[ 'name' ], $label_to_find ) == 0 ) {
				$logger->write( " - Name matches " );
				if ( abs( $rate_to_find - $tax_rate[ 'effective_rate' ] ) <= 0.0001 ) {
					$logger->write( " - Rate matches " );
